...
/**
 * Adds an inactivity boundary on top of the normal absolute session expiry.
 * The existing auth layer still owns token verification; this wrapper only
 * refuses sessions that have been idle for too long before delegating to it.
 */
export async function getSessionUserHardened(request: Request, env: AppEnv): Promise<SessionUser | null> {
  ...
  const row = await env.DB.prepare(
    'SELECT user_id, last_seen_at FROM sessions WHERE id = ? AND expires_at > ? LIMIT 1'
  ).bind(id, now).first<{ user_id: string; last_seen_at: number }>();
  if (!row) return null;
...
export async function revokeUserSession(
  ...
  if (currentId && sessionId === currentId) {
    return json({ error: 'Use Sair para encerrar a sessão atual.' }, 400);
  }
...
/**
 * Password changes are a session-rotation event. The current token is replaced
 * and every other session for the account is revoked, limiting the usefulness
 * of a previously stolen session token after a credential change.
 */
export async function changePasswordHardened(
  request: Request,
  env: AppEnv,
  user: SessionUser
): Promise<Response> {
  ...
  const session = await env.DB.prepare(
    'SELECT expires_at FROM sessions WHERE id = ? AND user_id = ? LIMIT 1'
  ...
  await env.DB.batch([
    ...
    env.DB.prepare('UPDATE sessions SET id = ?, last_seen_at = ? WHERE id = ? AND user_id = ?')
      .bind(newId, now, oldId, user.id),
    env.DB.prepare(
      'INSERT INTO audit_logs (id, user_id, action, target_type, target_id, created_at) VALUES (?, ?, ?, ?, ?, ?)'
